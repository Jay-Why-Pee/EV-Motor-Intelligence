...
export const TechnologyTrendChart = ({ data }: TechnologyTrendChartProps) => {
  ...
  return (
    <Card className="p-4 md:p-6 card-glow">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">채택률 vs 성장률 (뉴스 기반)</p>
      </div>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
          <XAxis dataKey="tech" stroke="hsl(var(--muted-foreground))" style={{ fontSize: '12px' }} />
          <YAxis stroke="hsl(var(--muted-foreground))" style={{ fontSize: '12px' }} />
          <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '8px', color: 'hsl(var(--foreground))' }} />
          <Legend wrapperStyle={{ fontSize: '12px' }} formatter={(value) => value === 'adoption' ? '현재 채택률 (%)' : '연간 성장률 (%)'} />
