...
export const MarketSizeChart = ({ data }: MarketSizeChartProps) => {
  ...
  return (
    <Card className="p-4 md:p-6 card-glow">
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground/70 mt-1">출처: 뉴스 기반 AI 분석</p>
      </div>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          <Legend wrapperStyle={{ fontSize: '12px' }} formatter={(value) => value === 'market' ? '실제 시장 규모' : '예측 시장 규모'} />
          <Line type="monotone" dataKey="market" stroke="hsl(var(--chart-1))" strokeWidth={3} dot={{ fill: 'hsl(var(--chart-1))', r: 4 }} activeDot={{ r: 6 }} />
          <Line type="monotone" dataKey="forecast" stroke="hsl(var(--chart-2))" strokeWidth={2} strokeDasharray="5 5" dot={{ fill: 'hsl(var(--chart-2))', r: 3 }} />
        </LineChart>
      </ResponsiveContainer>
    </Card>
  );
