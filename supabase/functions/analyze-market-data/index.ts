...
KPI는 정확히 4개. marketSize는 2020~2028년. regionalShare는 5개 지역. technologyTrend는 4~5개.`,
    ...
    const researchData = await callAI(
      `당신은 전기차 모터 기술 연구 전문가입니다. 뉴스에서 언급된 기술 트렌드를 기반으로 관련 연구 논문 정보를 생성하세요.
반드시 다음 JSON 구조로 응답:
{
  "papers": [
...
- insights: 전체 연구 동향에 대한 1~5개 핵심 인사이트
- link는 IEEE, ScienceDirect 등 실제 검색 URL`,
    ...
    const patentsData = await callAI(
      `당신은 전기차 모터 특허 분석 전문가입니다. 뉴스에서 언급된 기업과 기술을 기반으로 특허 정보를 생성하세요.
반드시 다음 JSON 구조로 응답:
{
  "patents": [
...
- insights: 전체 특허 동향에 대한 1~5개 핵심 인사이트
